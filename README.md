...
cargo build --release
```
Built binary can be found under `target/release/git-user`. Add `git-user` binary to PATH so `git` can recognise it as a subcommand.
...
Options:
  -c, --config <CONFIG>  Custom config file path [default: ~/.git_users]
  -h, --help             Print help
```
